...
import { Book, Package, AlertCircle, Loader2, Download, CheckCircle, Plus, Trash2 } from 'lucide-react';
        ...
        <div className="h-full overflow-y-auto scroll-container pr-2 relative">
            ...
            {showUninstallConfirm && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-dark-800 border border-dark-600 rounded-xl p-6 max-w-sm w-full shadow-2xl">
                        <h3 className="text-lg font-bold text-gray-800 mb-2">Uninstall Skill?</h3>
                        <p className="text-gray-500 text-sm mb-6">
                            Are you sure you want to verify removing <span className="text-gray-800 font-mono">{showUninstallConfirm}</span>? This action cannot be undone.
                        </p>
                        <div className="flex gap-3 justify-end">
                            <button
                                onClick={() => setShowUninstallConfirm(null)}
                                className="px-4 py-2 text-gray-600 hover:text-gray-900 hover:bg-dark-700 rounded-lg transition-colors text-sm"
            ...
            <div className="flex items-center justify-between mb-8">
                <div>
                    ...
                    <p className="text-gray-500">Manage your OpenClaw skills</p>
                </div>
                <button
            ...
            {installResult && (
                <div className={`rounded-xl p-4 mb-6 flex items-center gap-3 ${installResult.success ? 'bg-green-500/10 border border-green-500/50' : 'bg-red-500/10 border border-red-500/50'
                    ...
                    <p className={`text-sm ${installResult.success ? 'text-green-600' : 'text-red-600'}`}>
            ...
            {showInstallDialog && (
                <div className="bg-dark-700 rounded-2xl border border-dark-600 p-6 mb-8">
                    <h3 className="text-lg font-semibold text-gray-800 mb-4">Install New Skill</h3>
                    <div className="flex gap-2">
                        ...
                        <button
                            ...
                            className="px-6 py-2 bg-claw-500 hover:bg-claw-600 text-white rounded-xl transition-colors disabled:opacity-50 flex items-center gap-2"
                        >
                        ...
                        <button
                            ...
                            disabled={installingSkill}
                            className="px-4 py-2 text-gray-500 hover:text-gray-900 hover:bg-dark-600 rounded-xl transition-colors"
                        >
                            Cancel
                        </button>
                    </div>
                    <p className="mt-2 text-xs text-gray-500">
                        The skill will be installed and available for use.
                    </p>
                </div>
            )}
                        ...
                        >
                            <div className="flex items-start gap-4">
                                ...
                                <div className="flex-1 min-w-0">
                                    <h3 className="font-semibold text-gray-800 truncate">{skill.name}</h3>
                                    {skill.description && (
